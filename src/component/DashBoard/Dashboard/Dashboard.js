...
export default function AppointmentDashboard() {
  ...
  useEffect(() => {
    getAllResults(apiUrl + 'appointments/Booked-Appointments/');
  }, []);
  ...
  const upComingData = allResults.filter(
    element => element.status === 'waiting' || element.status === 'pending'
  );
  const numberOfWaitingRequests = upComingData.length;

  const previousData = allResults.filter(
